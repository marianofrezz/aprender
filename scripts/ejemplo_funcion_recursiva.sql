...
CREATE OR REPLACE FUNCTION eaprender.concatenartelefonospersona(v_id_persona bigint, v_min_id_telefono bigint) RETURNS VARCHAR
    AS $$
    ...
    RETURN (SELECT tels.nro_telefono::VARCHAR
              || CASE WHEN v_cant_tels > 1
                      THEN '; ' || eaprender.concatenartelefonospersona(v_id_persona, v_id_tel) 
                      ELSE '' END
         FROM eaprender.telefonos tels
        WHERE tels.id_telefono = v_id_tel); END $$
    LANGUAGE plpgsql;

SELECT *, eaprender.concatenarTelefonosPersona(pers.id_persona::BIGINT, NULL) telefonos_concatenados FROM eaprender.personas pers
